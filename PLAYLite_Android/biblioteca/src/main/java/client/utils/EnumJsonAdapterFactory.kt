...
/** Фабрика для обработки enum классов. См. [EnumIntSafeJsonAdapter]. */
object EnumJsonAdapterFactory : JsonAdapter.Factory
{
    override fun create(type: Type, annotations: Set<Annotation>, moshi: Moshi): JsonAdapter<*>? {
        ...
        return EnumIntSafeJsonAdapter(type as Class<Enum<*>>).nullSafe()
    }
}

/** Допускает числовые значения типа @Json(name = "128") для enum классов. */
internal class EnumIntSafeJsonAdapter<T : Enum<T>>(private val enumType: Class<T>) : JsonAdapter<T>()
{
